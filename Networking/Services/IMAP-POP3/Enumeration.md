...
> - The e-mail server belongs to the organization `Inlanefreight`
...
With the access credentials, an attacker could log in to the mail server and
read (or even send) the individual messages
...
To interact with the **IMAP** or **POP3** server over SSL, `openssl` and `ncat`
can be used.
